...
In this app, the navigation was wrapped in multiple context providers:
```tsx
<trpc.Provider>
  <QueryClientProvider>
    <GestureHandlerRootView>
      <AuthProvider>
        <SubscriptionProvider>
          <AuthWrapper>
            <Stack>  {/* Navigation here was too deeply nested */}
```
...
4. **Provider Context**: Ensure all context providers (auth, subscription, tRPC) work correctly
...
2. `hooks/auth-context.tsx` - Simplified login, register, and resendConfirmation functions
3. `components/auth/LoginScreen.tsx` - Removed email confirmation workarounds
4. `components/auth/RegisterScreen.tsx` - Simplified registration flow
